...
import { NitMinerUser, redirectToNitMiner, clearSession } from '@/lib/trustinn/jwtAuth';
...
export function SessionCheckModal({ isOpen, user, onClose, onRefresh }: SessionCheckModalProps) {
  ...
  const handleRefresh = async () => {
    ...
    setSyncError(null);
    try {
      console.log('[SessionCheckModal] Syncing session with NextAuth...');
      ...
      if (response.ok) {
        ...
        if (userData) {
          ...
          setLastUpdated(new Date());
          setSyncError(null);
          if (onRefresh) onRefresh(userData);
        } else {
          setSyncError('No user found in active session. Please login again.');
        }
      } else {
