...
import { initializeAuth, getStoredSessionToken, validateToken, consumeTrial,
  clearSession, redirectToNitMiner, fetchUserFromDB, fetchTrialCountByEmail } from '@/lib/trustinn/jwtAuth';
import NoAccessError from '@/components/trustinn/NoAccessError';
import { toolsPageTourSteps } from '@/lib/trustinn/tourSteps';

const CodeEditor   = lazy(() => import('@/components/trustinn/CodeEditor'));
const TourGuide    = lazy(() => import('@/components/trustinn/TourGuide'));
const AuthModal    = lazy(() => import('@/components/trustinn/AuthModals').then(m => ({ default: m.AuthModal })));
const SessionCheckModal = lazy(() => import('@/components/trustinn/AuthModals').then(m => ({ default: m.SessionCheckModal })));
const OTPVerificationModal = lazy(() => import('@/components/trustinn/OTPModal').then(m => ({ default: m.OTPVerificationModal })));
  ...
  const compileCode = async (language: 'c' | 'java' | 'python' | 'solidity') => {
    ...
    try {
      ...
      setTerminalOutputs(prev => ({
        ...
        [language]:
          (prev[language] || '') +
          '\n⚠️ Compile-only endpoint is not enabled in this integration.\nUse Execute to run full analysis.\n',
      }));
    } catch (e: any) {
      setTerminalOutputs(prev => ({
        ...prev,
